...
int main(int argc, char *argv[]) {
    ...
    int pCount; // Number of processors
    Int tCount; // number of tasks
    Int eCount; // number of directed edges

    runNotData(inputFile);
    inputFile >> pCount >> tCount >> eCount;

    double transDataRate[pCount][pCount]; // Trans-unit cost between processors
    double runCost[tCount][pCount]; // The time the processor executes each task
    double transDataVol[tCount][tCount];  //Data Units Transferred Between Processors
